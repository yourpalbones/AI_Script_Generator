...
class ScriptWriterApp:
    ...
    def setup_ui(self):
        ...
        # Sort options
        tk.Label(search_frame, text="Sort by:", 
                font=('Arial', 12), fg='#ffffff', bg='#2b2b2b').pack(side=tk.LEFT, padx=(20, 5))
        
        self.sort_var = tk.StringVar(value="Newest First")
        sort_dropdown = ttk.Combobox(search_frame, textvariable=self.sort_var,
                                    values=["Newest First", "Oldest First"], 
                                    state="readonly", width=15)
        sort_dropdown.pack(side=tk.LEFT, padx=(0, 10))
        sort_dropdown.bind('<<ComboboxSelected>>', self.on_sort_changed)
        ...
        # Headline display area
        headline_frame = tk.Frame(topics_frame, bg='#2b2b2b')
        headline_frame.pack(fill=tk.X, pady=(0, 10))
        
        tk.Label(headline_frame, text="Selected Headline:", 
                font=('Arial', 12, 'bold'), fg='#ffffff', bg='#2b2b2b').pack(anchor=tk.W)
        
        self.headline_display = tk.Label(headline_frame, text="Click on a topic to see full headline", 
                                        font=('Arial', 11), fg='#cccccc', bg='#2b2b2b',
                                        wraplength=1000, justify=tk.LEFT)
        self.headline_display.pack(fill=tk.X, pady=(5, 0))
        ...
        # Bind single-click to show headline and double-click to generate script
        self.topics_listbox.bind('<Button-1>', self.on_topic_single_click)
        ...
        self.make_facebook_btn = tk.Button(script_frame, text="Make Facebook Post", 
                                          ...
                                          padx=20, pady=5, state=tk.DISABLED)
        self.make_facebook_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        self.make_meme_btn = tk.Button(script_frame, text="Make a Meme", 
                                      command=self.make_meme,
                                      bg='#FF6B35', fg='white', font=('Arial', 12, 'bold'),
                                      padx=20, pady=5, state=tk.DISABLED)
        self.make_meme_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        self.make_poll_btn = tk.Button(script_frame, text="Make a Poll", 
                                      command=self.make_poll,
                                      bg='#9C27B0', fg='white', font=('Arial', 12, 'bold'),
                                      padx=20, pady=5, state=tk.DISABLED)
        self.make_poll_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        self.make_joke_btn = tk.Button(script_frame, text="Make a Quick Joke", 
                                      command=self.make_joke,
                                      bg='#FF9800', fg='white', font=('Arial', 12, 'bold'),
                                      padx=20, pady=5, state=tk.DISABLED)
        self.make_joke_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        self.view_source_btn = tk.Button(script_frame, text="View Source", 
                                       command=self.view_source,
                                       bg='#607D8B', fg='white', font=('Arial', 12, 'bold'),
                                       padx=20, pady=5, state=tk.DISABLED)
        self.view_source_btn.pack(side=tk.LEFT, padx=(0, 5))
    ...
    def _update_topics_display(self):
        ...
        # Sort topics based on current sort setting
        sorted_topics = self._sort_topics(self.current_topics)
        ...
        # Enable all buttons when topics are loaded
        self.make_script_btn.config(state=tk.NORMAL)
        self.make_facebook_btn.config(state=tk.NORMAL)
        self.make_meme_btn.config(state=tk.NORMAL)
        self.make_poll_btn.config(state=tk.NORMAL)
        self.make_joke_btn.config(state=tk.NORMAL)
    ...
    def _sort_topics(self, topics):
        """Sort topics based on current sort setting"""
        sort_order = self.sort_var.get()
        if sort_order == "Newest First":
            return sorted(topics, key=lambda x: x['timestamp'], reverse=True)
        else:  # Oldest First
            return sorted(topics, key=lambda x: x['timestamp'], reverse=False)
    ...
    def _get_selected_topic(self):
        """Get the currently selected topic, accounting for filtering and sorting"""
        ...
        if search_term:
            ...
            sorted_topics = self._sort_topics(filtered_topics)
            return sorted_topics[topic_index]
        else:
            sorted_topics = self._sort_topics(self.current_topics)
            return sorted_topics[topic_index]
    ...
    def filter_topics(self, *args):
        ...
        sorted_filtered_topics = self._sort_topics(filtered_topics)
        
        self.topics_listbox.delete(0, tk.END)
        for topic in sorted_filtered_topics:
            display_text = f"{topic['title']} | {topic['source']} | {topic['time_ago']} | {topic['timestamp']}"
            self.topics_listbox.insert(tk.END, display_text)
    
    def on_topic_single_click(self, event):
        """Handle single-click to show full headline"""
        topic = self._get_selected_topic()
        if topic:
            # Update headline display
            self.headline_display.config(text=topic['title'])
    ...
    def make_meme(self):
        ...
        # Generate meme in separate thread
        self.make_meme_btn.config(state=tk.DISABLED, text="Generating Meme...")
        self.progress.start()
        
        thread = threading.Thread(target=self._generate_meme, args=(topic,))
        thread.daemon = True
        thread.start()
    ...
    def make_poll(self):
        ...
        # Generate poll in separate thread
        self.make_poll_btn.config(state=tk.DISABLED, text="Generating Poll...")
        self.progress.start()
        
        thread = threading.Thread(target=self._generate_poll, args=(topic,))
        thread.daemon = True
        thread.start()
    ...
    def make_joke(self):
        ...
        # Generate joke in separate thread
        self.make_joke_btn.config(state=tk.DISABLED, text="Generating Joke...")
        self.progress.start()
        
        thread = threading.Thread(target=self._generate_joke, args=(topic,))
        thread.daemon = True
        thread.start()
    ...
    def view_source(self):
        ...
        url = topic.get('url', '')
        if not url or url == '#':
            messagebox.showinfo("No Source Available", "This topic doesn't have a source URL available.")
            return
        
        try:
            # Open the URL in the default browser
            webbrowser.open(url)
            self.status_label.config(text=f"Opened source: {topic['source']}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to open source URL: {str(e)}")
    
    def _generate_meme(self, topic):
        try:
            meme = self.chatgpt.generate_meme(topic, self.settings_manager)
            self.root.after(0, lambda: self._show_meme(meme, topic))
        except Exception as e:
            self.root.after(0, lambda: self._show_error(f"Error generating meme: {str(e)}"))
        finally:
            self.root.after(0, self._meme_generation_finished)
    
    def _generate_poll(self, topic):
        try:
            poll = self.chatgpt.generate_poll(topic, self.settings_manager)
            self.root.after(0, lambda: self._show_poll(poll, topic))
        except Exception as e:
            self.root.after(0, lambda: self._show_error(f"Error generating poll: {str(e)}"))
        finally:
            self.root.after(0, self._poll_generation_finished)
    
    def _generate_joke(self, topic):
        try:
            joke = self.chatgpt.generate_joke(topic, self.settings_manager)
            self.root.after(0, lambda: self._show_joke(joke, topic))
        except Exception as e:
            self.root.after(0, lambda: self._show_error(f"Error generating joke: {str(e)}"))
        finally:
            self.root.after(0, self._joke_generation_finished)
    
    def _meme_generation_finished(self):
        self.make_meme_btn.config(state=tk.NORMAL, text="Make a Meme")
        self.progress.stop()
    
    def _poll_generation_finished(self):
        self.make_poll_btn.config(state=tk.NORMAL, text="Make a Poll")
        self.progress.stop()
    
    def _joke_generation_finished(self):
        self.make_joke_btn.config(state=tk.NORMAL, text="Make a Quick Joke")
        self.progress.stop()
    
    def _show_meme(self, meme, topic):
        # Create new window for meme display
        meme_window = tk.Toplevel(self.root)
        meme_window.title(f"Meme: {topic['title'][:50]}...")
        meme_window.geometry("600x500")
        meme_window.configure(bg='#2b2b2b')
        
        # Title
        title_label = tk.Label(meme_window, text=topic['title'], 
                              font=('Arial', 14, 'bold'), 
                              fg='#ffffff', bg='#2b2b2b', wraplength=550)
        title_label.pack(pady=10)
        
        # Meme content
        meme_text = scrolledtext.ScrolledText(meme_window, 
                                             font=('Arial', 11),
                                             bg='#3b3b3b', fg='#ffffff',
                                             wrap=tk.WORD)
        meme_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        meme_text.insert(tk.END, meme)
        meme_text.config(state=tk.DISABLED)
        
        # Save button
        save_btn = tk.Button(meme_window, text="Save Meme", 
                            command=lambda: self._save_meme(meme, topic),
                            bg='#FF6B35', fg='white', font=('Arial', 12))
        save_btn.pack(pady=10)
    
    def _show_poll(self, poll, topic):
        # Create new window for poll display
        poll_window = tk.Toplevel(self.root)
        poll_window.title(f"Poll: {topic['title'][:50]}...")
        poll_window.geometry("600x500")
        poll_window.configure(bg='#2b2b2b')
        
        # Title
        title_label = tk.Label(poll_window, text=topic['title'], 
                              font=('Arial', 14, 'bold'), 
                              fg='#ffffff', bg='#2b2b2b', wraplength=550)
        title_label.pack(pady=10)
        
        # Poll content
        poll_text = scrolledtext.ScrolledText(poll_window, 
                                             font=('Arial', 11),
                                             bg='#3b3b3b', fg='#ffffff',
                                             wrap=tk.WORD)
        poll_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        poll_text.insert(tk.END, poll)
        poll_text.config(state=tk.DISABLED)
        
        # Save button
        save_btn = tk.Button(poll_window, text="Save Poll", 
                            command=lambda: self._save_poll(poll, topic),
                            bg='#9C27B0', fg='white', font=('Arial', 12))
        save_btn.pack(pady=10)
    
    def _show_joke(self, joke, topic):
        # Create new window for joke display
        joke_window = tk.Toplevel(self.root)
        joke_window.title(f"Joke: {topic['title'][:50]}...")
        joke_window.geometry("600x500")
        joke_window.configure(bg='#2b2b2b')
        
        # Title
        title_label = tk.Label(joke_window, text=topic['title'], 
                              font=('Arial', 14, 'bold'), 
                              fg='#ffffff', bg='#2b2b2b', wraplength=550)
        title_label.pack(pady=10)
        
        # Joke content
        joke_text = scrolledtext.ScrolledText(joke_window, 
                                             font=('Arial', 11),
                                             bg='#3b3b3b', fg='#ffffff',
                                             wrap=tk.WORD)
        joke_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        joke_text.insert(tk.END, joke)
        joke_text.config(state=tk.DISABLED)
        
        # Save button
        save_btn = tk.Button(joke_window, text="Save Joke", 
                            command=lambda: self._save_joke(joke, topic),
                            bg='#FF9800', fg='white', font=('Arial', 12))
        save_btn.pack(pady=10)
    
    def _save_meme(self, meme, topic):
        filename = f"meme_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f"Topic: {topic['title']}\n")
                f.write(f"Source: {topic['source']}\n")
                ...
                f.write("-" * 50 + "\n\n")
                f.write(meme)
            messagebox.showinfo("Saved", f"Meme saved as {filename}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save meme: {str(e)}")
    
    def _save_poll(self, poll, topic):
        filename = f"poll_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f"Topic: {topic['title']}\n")
                f.write(f"Source: {topic['source']}\n")
                ...
                f.write("-" * 50 + "\n\n")
                f.write(poll)
            messagebox.showinfo("Saved", f"Poll saved as {filename}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save poll: {str(e)}")
    
    def _save_joke(self, joke, topic):
        filename = f"joke_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f"Topic: {topic['title']}\n")
                f.write(f"Source: {topic['source']}\n")
                ...
                f.write("-" * 50 + "\n\n")
                f.write(joke)
            messagebox.showinfo("Saved", f"Joke saved as {filename}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to save joke: {str(e)}")
